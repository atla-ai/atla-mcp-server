    ...
    WithJsonSchema(
        {
            ...
            "examples": [
                ...
                "Can you summarize the main idea behind transformers in machine learning?",  # noqa: E501
            ],
        }
    ),
]

AnnotatedModelOutput = Annotated[
    str,
    WithJsonSchema(
        {
            "description": (
                "The output generated by the model in response to the `model_input`, "
                "which needs to be evaluated."
            ),
            "examples": [
                dedent(
                    """The Moon doesn't have a capital — it has no countries, governments,
    ...
    WithJsonSchema(
        {
            "description": (
                ...
                "task. Typical rubrics score response either on a Likert scale from "
...
AnnotatedExpectedModelOutput = Annotated[
    Optional[str],
    WithJsonSchema(
        {
            "description": (
                "A reference or ideal answer to compare against the `model_output`. "
...
AnnotatedModelContext = Annotated[
    Optional[str],
    WithJsonSchema(
        {
            "description": (
                ...
                "with additional information that is not part of the `model_input` "
                "or `expected_model_output` (e.g., a RAG retrieval context). "
                "Defaults to None."
            )
        }
    ),
    ...
    WithJsonSchema(
        {
            "description": (
                ...
                "flagship Atla model, optimized for the highest all-round "
                "performance. `atla-selene-mini` is a compact model that is "
                "generally faster and cheaper to run. Defaults to `atla-selene`."
            )
        }
    ),
]
...
async def evaluate_llm_response(
    ...
    llm_prompt: AnnotatedModelInput,
    llm_response: AnnotatedModelOutput,
    expected_model_output: AnnotatedExpectedModelOutput = None,
    model_context: AnnotatedModelContext = None,
    model_id: AnnotatedModelId = "atla-selene",
) -> dict[str, str]:
    """Evaluate an LLM's response to an input prompt using a given evaluation criterion.
...
async def evaluate_llm_response_on_multiple_criteria(
    ...
    """Evaluate an LLM's response to an input prompt across *multiple* evaluation criteria.
